  ...
  const saveCupom = async (cupomData: CupomFiscal, itens?: ItemCompra[]): Promise<boolean> => {
    ...
    try {
      ...
      if (existingCupom) {
        ...
        toast.custom((t) => (
          <div className="bg-white shadow rounded p-4 text-sm text-gray-800">
            <span>Este QR code já foi escaneado anteriormente.</span>
            <button
              className="ml-2 bg-green-500 text-white px-2 py-1 rounded"
              onClick={ async() => {
                toast.dismiss(t.id);
                deleteCupom(existingCupom.id, false); // sua função de update
...
const deleteCupom = async (id: number, confirmar: boolean = true) => {
  if (!confirmar) {
    // Exclusão direta, sem pedir confirmação
    try {
      const { error } = await supabase
        .from('cupom_fiscal')
        .delete()
  ...
  // Se confirmar for true, mostra o toast interativo
  toast.custom((t) => (
    <div className="bg-white border border-gray-200 p-4 rounded shadow text-sm text-gray-800 max-w-sm">
      <p className="mb-2">Tem certeza que deseja excluir este cupom fiscal?</p>
